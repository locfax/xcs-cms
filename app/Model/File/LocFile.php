...
class LocFile {
    ...
    public function check($fileField, $isOne = true, $filesite = false, $prefix = null) {
        //文件检测
        $uploader = \Xcs\Helper\Uploader::getInstance()->init($_FILES);
        ...
        if (!$_filedir) {
            $ERROR = '路径错误' . $_filedir;
            return array('flag' => false, 'msg' => $ERROR);
        }
        $allowfile = array(
        ...
        do {
            if ($isOne) {
                ...
                return array('flag' => true, 'file' => $file, 'isimage' => $isimage, 'path' => $path, 'prefix' => $_prefix, 'filesite' => $_filesite);
            } else {
                ...
                foreach ($files as $file) {
                    ...
                    $truefile[] = array('file' => $file, 'isimage' => $isimage, 'path' => $path, 'prefix' => $_prefix, 'filesite' => $_filesite);
                }
    ...
    //上传文件
    public function upload(array $objfile, $filename = '', $mode = array()) {
        $_newname = $filename;
        $path = $objfile['path'];
        ...
        if (!\Xcs\Helper\File::mk_dir($_fileDir) && !is_dir($_fileDir)) {
            return false; //生成路径
        }

        $default_mode = array('local' => 0, 'after' => 'unlink');
        if (!empty($mode)) {
        ...
        return array('flag' => $flag, 'sourcename' => $_filename, 'newname' => $_newname, 'filepath' => $filepath, 'filesize' => $size);
    }

    //生成缩略图
    public function thumb(array $objfile, $filename = '', $whg = array(), $mode = array()) {
        $_newname = $filename;
        $path = $objfile['path'];
        $prefix = $objfile['prefix'];
        $filesite = $objfile['filesite'];
        ...
        if (!\Xcs\Helper\File::mk_dir($_fileDir)) {
            return false;
        }
        $default_mode = array('mark' => 0, 'local' => false, 'imghandle' => 'imagegd', 'after' => '');
        if (!empty($mode)) {
            $mode = array_merge($default_mode, $mode);
        } else {
            $mode = $default_mode;
        ...
        $imagehelper = '\\Xcs\\Helper\\' . ucfirst($mode['imghandle']);
        if (!empty($whg) && is_array($whg)) {
            $WHgroup = $whg;
        } else {
            $WHgroup = array(array('min', '150', '150', 'crop'));
        }
        //缩略图组
        $biglast = $bigwidth = 0;
        foreach ($WHgroup as $WH) {
            ...
            $handfn = $WH[3];
            if ('crop' == $handfn) {
                $imghandle->crop($width, $height, array('fullimage' => true, 'enlarge' => false, 'reduce' => true));
                ...
                $imghandle->crop($width, $height, array('fullimage' => true, 'enlarge' => true, 'reduce' => true));
            }
        ...
        if ($bigwidth && $bigwidth > 150 && $mode['mark']) {
            ...
            if (is_file($newfile)) {
                ...
                \Xcs\Helper\WaterMark::mark($newfile, 9, $markfile, '水印');
            }
        }
        if ('unlink' == $mode['after']) {
            $file->remove();
        }
        return array('newname' => $_newname, 'filepath' => $filepath);
    }
