...
//CHECKBOX扩展分类 多选
    function select_list_muti($data, $extrafid = '', $level = 0, $idname = 'catid', $name = 'name', $fchildrens = 'childrens') {
        ...
        foreach ($data as $row) {
            ...
            if (\Xcs\Util::strpos($extrafid, "'" . $row[$idname] . "'")) {
...
//分类操作链接
    function _option_href($id, $classname, $controller, $extra, $pid) {
        $options = '&nbsp;&nbsp;';
        $options .= '&nbsp;&nbsp;<a href="' . url("$controller/add", array('id' => $id)) . '" title="新增下级分类" ' . $extra . '><i class="icon-plus"></i>新增</a>';
        $options .= ' <a href="' . url("{$controller}/edit", array('id' => $id)) . '" title="编辑该分类" ' . $extra . '><i class="icon-edit"></i>编辑</a> ';
        if ($pid) {
            $options .= ' <a href="' . url("{$controller}/del", array('id' => $id)) . '" onclick="return theList.confirm(this,\'确认认删除该分类吗？分类名称: ‘' . $classname . '’\')" title="删除该分类">删除</a>';
        }
        return $options;
    }
...
//分类操作链接
    function _post_href($id, $name, $ischannel) {
        if ($ischannel) {
            ...
            $options = '&nbsp;&nbsp;<a href="' . url('node/create', array('catid' => $id)) . '" title="' . $name . '">点击发布</a>';
        }
        return $options;
    }
...
//SELECT帮助中心下拉
    ...
    function _faq_href($id, $classname) {
        $options = '&nbsp;&nbsp;<a href="' . url('faq/add', array('fpid' => $id)) . '" onclick="return theList.edit(0, this, \'info\')" title="新增下级">新增</a>';
        $options .= ' <a href="' . url('faq/edit', array('id' => $id)) . '" onclick="return theList.edit(0, this, \'info\')" title="编辑">编辑</a> ';
        $options .= ' <a href="' . url('faq/remove', array('id' => $id)) . '" onclick="return theList.confirm(this, \'确认认删除吗？标题: ‘' . $classname . '’\')" title="删除">删除</a>';
        return $options;
    }
