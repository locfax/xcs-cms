...
class Tool extends Controller {
    ...
    function act_makeacl() {
        ...
        foreach ($ACL as $key => $val) {
            $_ACL[$key] = $this->_prepareACL($val);
        }
        $actdata = \Xcs\Util::output_json($_ACL);

        \Xcs\SysCache::save('sys_acl' . APPKEY, $actdata);

        \Xcs\Util::rep_send('it is OK', 'xml');
    }
